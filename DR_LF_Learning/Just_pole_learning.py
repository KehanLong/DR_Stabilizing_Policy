...
def train_clf_nn_controller():
    ...
    clf_controller = Just_Pole_Joint_Controller(net_nominal, net_policy, relaxation_penalty)
    ...
    num_angular = 80
    theta_dot = torch.Tensor(num_angular).uniform_(-8.0, 8.0)  # adjust this range if needed
    ...
    x_train_temp[:, 0] = torch.cos(x_original[:, 0])          #cos angle  
    x_train_temp[:, 1] = torch.sin(x_original[:, 0])          #sin angle  
    x_train_temp[:, 2] = x_original[:, 1]                     #angluar velocity
    ...
    torch.save(net_nominal.state_dict(), "saved_models/joint_clf_controller_models/just_pole/baseline_joint_clf_1.pt")
    torch.save(net_policy.state_dict(), "saved_models/joint_clf_controller_models/just_pole/baseline_controller_1.pt")
...
def evaluate_clf_controller(clf_controller, file_prefix='Cart Pole'):
    ...
    Theta_dot, Theta = np.meshgrid(position, theta)
    ...
    X_tensor = torch.Tensor(np.vstack([cos_theta, sin_theta, np.zeros_like(sin_theta)]).T).to(device)
    ...
    desired_state = torch.tensor([1., 0., 0.])
    ...
    violation_points = np.column_stack([Theta_dot[mask == 1], Theta[mask == 1]])
    ...
    contour = plt.contourf(Theta_dot, Theta, V_dot_plot, levels=10)
    ...
    # Plot the Lyapunov function derivative values
    contour = plt.contourf(Theta_dot, Theta, V_values, levels=20)
    ...
    plt.figure(figsize=(10, 8))
    contour = plt.contourf(Theta_dot, Theta, U_values, levels=20)
