...
def simulate_joint_trajectory(controller, policy, initial_state, time_steps=600, dt=0.05):
...
if __name__ == "__main__":
    ...
    clf_saved_model = "saved_models/joint_clf_controller_models/just_pole/baseline_joint_clf_1.pt"
    
    policy_saved_model = "saved_models/joint_clf_controller_models/just_pole/baseline_controller_1.pt"
    ...
    clf_controller = Just_Pole_Joint_Controller(net_nominal, net_policy, relaxation_penalty=2.0)
    ...
    converted_state = np.array([0.707, 0.707, 0])
    ...
    initial_states = [torch.tensor([np.cos(angles[i]), np.sin(angles[i]), 0.]) for i in range(10)]
    ...
    x_data_position_theta = [traj[:, 2] for traj in trajectories]
    y_data_position_theta = [np.arctan2(traj[:, 1], traj[:, 0]) for traj in trajectories]
    plot_trajectories(trajectories, x_data_position_theta, y_data_position_theta, 'Cart_Pole_Traj_1', 'Angular Velocity', 'Theta')
