...
class Just_Pole_Joint_Controller:
    def __init__(self, net, net_policy, relaxation_penalty = None):
        self.net = net
    ...
    def cart_pole_dynamics(self, x):
        ...
        f_x = torch.stack([-sin_theta * theta_dot, cos_theta * theta_dot, theta_double_dot], dim=1).to(x.device)
        g_x = torch.stack([torch.zeros_like(sin_theta), torch.zeros_like(sin_theta),   -cos_theta / denominator2], dim=1).to(x.device)
    
        return f_x, g_x
    
    def compute_policy(self, x, min_val=-150.0, max_val=150.0):
        """
    ...
    #vectorized version 
    def compute_clf(self, x, origin=[1, 0, 0], alpha=0.1):
