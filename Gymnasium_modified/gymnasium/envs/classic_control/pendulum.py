...
class PendulumEnv(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, g=9.81, m =1.0, l = 1.0, b =0.13, controller = 'Baseline', initial_state = np.array([np.pi, 0])):
        ...
        '''
        Modified dynamics model: include damping
    ...
    def step(self, u):
        ...
        newthdot = thdot + ( g * np.sin(th) / l - b * thdot / (m * l**2) + u / (m * l**2) ) * dt
        
        #no damping version
        #newthdot = thdot + ( g * np.sin(th) / l  + u / (m * l**2) ) * dt
    ...
    #     #newthdot = thdot + ( g * np.sin(th) / l + 1.0 / (m * l**2) * u) * dt
    ...
    def render(self):
        ...
        if self.last_u is not None:
            # Adjust the scale factor to reduce the size of the control indicator
            control_scale_factor = 0.3  # Adjust this value as needed
            scale_img = pygame.transform.smoothscale(
                img,
                (scale * np.abs(self.last_u) * control_scale_factor, scale * np.abs(self.last_u) * control_scale_factor),
            )
        ...
        if self.render_mode == "human":
            ...
            text_surface = font.render(f'Mass: {self.m}, Length: {self.l}, Damping: {self.b}', True, (0, 0, 0))
